      ...
      res.writeHead = function (status, headers) {
        ...
        if (!(resourceConfig.origins) || (resourceConfig.origins.indexOf(origin) !== -1)) {
          ...
          if (req.method === 'OPTIONS') {
            ...
            if (req.headers['access-control-request-method'] && resourceConfig.methods && resourceConfig.methods.indexOf(req.headers['access-control-request-method']) !== -1) {
              headers['Access-Control-Allow-Methods'] = req.headers['access-control-request-method'];
            }

            if (req.headers['access-control-request-headers'] && resourceConfig.headers && resourceConfig.headers.indexOf(req.headers['access-control-request-headers']) !== -1) {
